...
import edu.sru.thangiah.zeus.vrp.vrpqualityassurance.VRPQANode;

public class VRPBTWQATruck extends QATruck implements java.io.Serializable, java.lang.Cloneable
{
	public VRPBTWQATruck() {
	  }

	  public boolean checkDistanceConstraint(VRPBTWQATruck truck) {
		  boolean isDiagnostic = true;
		    ...
		    float floatDistance1;
		    float floatDistance12;
		    ...
		    if (isDiagnostic) {
		      ...
		      for (int i = 0; i < getNodes().size(); i++) {
		        node1 = (VRPBTWQANode) getNodes().elementAt(i);
		    ...
		    node1 = (VRPBTWQANode) getNodes().elementAt(0);   
		    //compute the total travel distance of the nodes in the route
		    for (int i = 1; i < getNodes().size(); i++) {
		      node2 = (VRPBTWQANode) getNodes().elementAt(i);
		      distance = (float) Math.sqrt( (float) (node1.getX() - node2.getX())
		    ...
		    /*node2 = (VRPBQANode) getNodes().elementAt(0);
		    ...
		    */
		    ...
		    //place
		    floatDistance1 = (float) (truck.getDistance() * 1000);
		    floatDistance12 = (float) (totalDistance * 1000);
		    ...
		    if (floatDistance1 != floatDistance12) {
	  ...
	  public boolean checkCapacityConstraint(VRPBTWQATruck truck) {
	    boolean isDiagnostic = true;
	    boolean status = true;
	    ...
	    if (isDiagnostic) {
	      ...
	      System.out.println("Sequence of the route is:");
	      for (int i = 0; i < getNodes().size(); i++) {
	        node = (VRPBTWQANode) getNodes().elementAt(i);
